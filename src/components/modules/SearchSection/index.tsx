...
import { SearchTap as SearchTapType, SearchTapLabel, UserDetails, RepositoryDetails } from '@/types';
...
function SearchSection() {
    // State
    const [activeTap, setActiveTap] = useState<SearchTapLabel>(SearchTapLabel.Users)
    const [searchTerm, setSearchTerm] = useState<string>('')
    const [searchUsersResults, setSearchUsersResults] = useState<UserDetails[]>([])
    const [searchReposResults, setSearchReposResults] = useState<RepositoryDetails[]>([])
    const [loading, setLoading] = useState<boolean>(false)
    const [searchError, setSearchError] = useState<boolean>(false)
    const [hasMoreItems, setHasMoreItems] = useState<boolean>(false)

    const [debounceSearchTerm] = useDebounce(searchTerm);
    const { page, reset, lastNodeRef } = useInfiniteScroll(loading, hasMoreItems, searchError)
    ...
    const renderSearchTaps = () => {
        ...
        return (
            <div className={styles['tabs-container']}>
                {searchTaps.map(tap => <SearchTap key={tap.label} {...tap} onTapSelect={onTapSelect} />)}
            </div>
        )
    }
        ...
        try {
            if (debounceSearchTerm.length > 3) {
                if (activeTap === SearchTapLabel.Users) {
                    const { data } = await axios.get(`${config.api.routes.users}?searchTerm=${debounceSearchTerm}&limit=${PAGE_LIMIT}&page=${page}`)
                    if (data?.length !== 0) {
                    ...
                    const { data } = await axios.get(`${config.api.routes.repositories}?searchTerm=${debounceSearchTerm}&limit=${PAGE_LIMIT}&page=${page}`)
    ...
    return (
        <div className={styles.container}>
            ...
            <div>
                {(activeTap === SearchTapLabel.Users ? searchUsersResults : searchReposResults).map((user, index, self) =>
                    <div style={{ height: '50px', backgroundColor: 'lightblue', color: 'white', borderBottom: '1px solid black' }} key={user.id} ref={self.length === index + 1 ? lastNodeRef : null}>{user.name}</div>
                )}
            </div>
            <div>{loading && 'loading ...'}</div>
            {searchError && <div style={{ color: 'red' }}>
                <p>{'Something went wrong!'}</p>
                <button onClick={fetchGithubInfo}>Retry</button>
            </div>}

            {(searchTerm.length > 3 && !(searchUsersResults.length || searchReposResults.length) && !searchError && !loading) && <div style={{ backgroundColor: 'gray' }} >No Data!</div>}
